...
class ProductItem {
  ...
  // Method that adds a product object into the shopping cart
  addTocart() {
    console.log("Adding a product to the cart...");
    console.log(this.product);
  }

  // Method that render a product object as a DOM element then return it to the calling statement
  render() {
    ...
    // Select the <button> element from productElement's rendered innerHTML
    const addCartButton = productElement.querySelector("button");

    // Add a corresponding event listener; binds the addToCart() method to the object ProductItem using .bind(this)
    // Without .bind(this), the method "binds" the value of "this" to the global Window object
