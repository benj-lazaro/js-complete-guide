...
class ProductItem extends Component {
  ...
  constructor(product, renderHookId) {
    // Prevents the Base Class' Constructor method from calling Sub-Class' method "render()"
    super(renderHookId, false);

    // Assign the current "ProductItem" object read as value
    this.product = product;

    // Call its method(s) locally
    this.render();
  }
...
class ProductList extends Component {
  // Convert to a private Class property
  #products = [];
  ...
  // Method that simulate a data fetch from a remote server
  fetchProducts() {
    this.#products = [
      new Product(
        "A Pillow",
    ...
    // Since the Class property "products" has assigned values, call method "renderProducts()"
    this.renderProducts();
  }

  // Method that iterate through each object element stored in the property "products"
  renderProducts() {
  ...
  render() {
    // Use inherited method to create & set an Element node "ul"
    this.createRootElement("ul", "product-list", [
      new ElementAttribute("id", "prod-list"),
    ]);

    // Prevents calling the method "renderProducts()" for the 1st time
...
class ShoppingCart extends Component {
  ...
  constructor(renderHookId) {
    ...
    // Callback function w/in a property for the Event "click" w/in the method "render()"
    this.orderProducts = () => {
      console.log("Ordering...");
      console.log(this.items);
    };

    // Manually call the Class method "render()"
    this.render();
  }
