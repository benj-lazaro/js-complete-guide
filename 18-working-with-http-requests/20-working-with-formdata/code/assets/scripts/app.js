...
function sendHttpRequest(method, url, data) {
  // Returns the promised (parsed) HTTP response
  return fetch(url, {
    method: method,
    body: data,
  })
    .then((response) => {
      ...
      if (response.status >= 200 && response.status < 300) {
        // Return the streamed HTTP response as parsed
        return response.json();
      } else {
        return response.json().then((errorData) => {
          // Display extra error data returned from the remote server
...
async function createPost(title, content) {
  ...
  // Prepare the key-value pairs for the FormData object to be submitted
