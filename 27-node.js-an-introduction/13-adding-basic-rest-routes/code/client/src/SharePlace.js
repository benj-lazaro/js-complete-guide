...
class PlaceFinder {
  ...
  selectPlace(coordinates, address) {
    ...
    // Send custom HTTP request to the local server
    fetch("http://localhost:3000/add-location/", {
      ...
      .then((response) => {
        // Receive the returned HTTP response from the server
        return response.json();
      })
      .then((data) => {
        // Extract data from the HTTP response & display it on the browser's JavaScript console
        console.log(data);
      });
  ...
  // Event Handler (method) that copy the shared link to the system clipboard
  sharePlaceHandler() {
    const sharedLinkInputElement = document.getElementById("share-link");

    // Check for clipboard API support
    if (!navigator.clipboard) {
      // Selects the "share link" field; allows the user to manually copy the full URL via CTRL+C
      sharedLinkInputElement.select();
      return;
    }

    // Write (copy) the full URL into the system clipboard
    navigator.clipboard
